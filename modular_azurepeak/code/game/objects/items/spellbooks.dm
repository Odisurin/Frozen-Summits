				...
				if(isarcyne(user))
					...
					if(prob(1))
						...
						user.visible_message(span_warning("[user] crushes [user.p_their()] [P]! Its powder just kind of sits on top of the [src]. Awkward."), \
							span_notice("... why and how did I just crush this gem into a worthless scroll-book? What a WASTE of coin!"))
						qdel(P)
					return ..()
		else
				...
				if(do_after(user, crafttime, target = src))
					if (isarcyne(user))
						...
						if (prob(the_rock.magic_power)) // for reference, this is never higher than 15 and usually significantly lower
							...
							span_notice("I knew this stone was special! Its colourful magic has soaked into my tome and given me gift of mystery!"))
